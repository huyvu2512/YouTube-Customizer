...
(function() {
    ...
    console.log('[YouTube Customizer] Script logic loaded successfully by Tampermonkey.');

    // ----- THAY ĐỔI SỐ CỘT VIDEO TẠI ĐÂY -----
    const ITEMS_PER_ROW = 4; // Thay đổi số 4 thành số cột bạn muốn
    // ------------------------------------------

    // CSS để điều chỉnh số cột video
    function addCustomStyles() {
        const style = document.createElement('style');
        style.type = 'text/css';
        style.textContent = `
            /* Điều chỉnh số cột video */
            ytd-rich-grid-renderer {
                --ytd-rich-grid-items-per-row: ${ITEMS_PER_ROW} !important;
            }
        `;
        document.head.appendChild(style);
        console.log('[YouTube Customizer] Custom styles for columns applied.');
    }

    // Đảm bảo CSS được thêm vào khi DOM sẵn sàng
    ...
    // Chờ trang tải hoàn tất để bắt đầu tìm và thay đổi logo
    window.addEventListener('load', () => {
        console.log('[YouTube Customizer] Page fully loaded. Starting logo search.');

        // Hàm thay thế mã SVG của logo
        function modifyYtIcon(logoElement) {
            console.log('[YouTube Customizer] Modifying logo icon.');
            const ytdLogoSvg = logoElement.querySelector("svg");
            if (ytdLogoSvg) {
                ...
                console.log('[YouTube Customizer] SVG element not found inside logo container.');
            }
        }

        // Hàm kiểm tra sự tồn tại của logo
        function checkYtIconExistence() {
            // Selector mới, trực tiếp hơn để tìm logo
            const logoElement = document.querySelector("ytd-logo #logo-icon, ytm-logo #logo-icon"); // Hỗ trợ cả Desktop và Mobile

            if (logoElement) {
                console.log('[YouTube Customizer] Logo element found!');
                modifyYtIcon(logoElement);
                // Ngừng theo dõi khi đã tìm thấy và thay đổi logo
                observer.disconnect();
                console.log('[YouTube Customizer] Observer disconnected.');
            }
        }

        // Theo dõi các thay đổi trên trang để tìm logo
        ...
        // Chạy kiểm tra một lần ngay khi bắt đầu
